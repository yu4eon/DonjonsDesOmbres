...
public class Joyau : MonoBehaviour
{
    ...
    [SerializeField] AudioClip _sonJoyaux; // Son quand le joueur ramasse le joyau #synthese Antoine
    ...
    void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.CompareTag("Player")) //Si le joueur entre en collision avec le joyau
        {
            ...
            Retroaction retro = Instantiate(_retroModele, transform.position, Quaternion.identity, transform.parent); // Crée une rétroaction
            ...
            Destroy(gameObject);
            GestAudio.instance.JouerEffetSonore(_sonJoyaux); // Joue le son du joyau #synthese Antoine
        }
    }
}
